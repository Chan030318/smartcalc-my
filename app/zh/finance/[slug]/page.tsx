...
export function generateStaticParams() {
  return getArticlesByCategory("finance").map((a) => ({ slug: a.slug }));
}
...
export async function generateMetadata({
  ...
  const url = `${BASE_URL}/zh/finance/${slug}`;
  const canonicalEn = `/finance/${slug}`;
  ...
  return {
    ...
    alternates: {
      ...
      languages: {
        "en-MY": `${BASE_URL}/finance/${slug}`,
        "ms-MY": `${BASE_URL}/finance/${slug}`,
        "zh-MY": url,
      },
    },
    openGraph: {
...
export default async function ZhFinanceArticlePage({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
    ...
    "@graph": [
      ...
      },
      {
        ...
        itemListElement: [
          ...
          { "@type": "ListItem", position: 2, name: "财商", item: `${BASE_URL}/zh/finance` },
