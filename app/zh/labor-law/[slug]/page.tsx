...
export function generateStaticParams() {
  return getArticlesByCategory("labor-law").map((a) => ({ slug: a.slug }));
}
...
export async function generateMetadata({
  ...
  const article = getArticle("labor-law", slug);
  if (!article) return {};

  const url = `${BASE_URL}/zh/labor-law/${slug}`;
  const canonicalEn = `/labor-law/${slug}`;
  ...
  return {
    ...
    alternates: {
      ...
      languages: {
        "en-MY": `${BASE_URL}/labor-law/${slug}`,
        "ms-MY": `${BASE_URL}/labor-law/${slug}`,
        "zh-MY": url,
      },
    },
    openGraph: {
...
export default async function ZhLaborLawArticlePage({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
    ...
    "@graph": [
      ...
      },
      {
        ...
        itemListElement: [
          ...
          { "@type": "ListItem", position: 2, name: "劳动法", item: `${BASE_URL}/zh/labor-law` },
