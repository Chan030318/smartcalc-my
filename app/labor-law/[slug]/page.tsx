...
export function generateStaticParams() {
  return getArticlesByCategory("labor-law").map((article) => ({
    slug: article.slug,
  }));
}
...
export async function generateMetadata({
  ...
  const url = `${BASE_URL}/labor-law/${slug}`;
  ...
  return {
    title,
    description: desc,
    alternates: { canonical: `/labor-law/${slug}` },
    openGraph: {
      title,
      description: desc,
      url,
...
export default async function LaborLawArticlePage({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
    ...
    "@graph": [
      ...
      },
      {
        ...
        itemListElement: [
          ...
          { "@type": "ListItem", position: 2, name: "Labor Rights", item: `${BASE_URL}/labor-law` },
